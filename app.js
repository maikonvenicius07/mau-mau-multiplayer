...
$('#doubleActionBtn').onclick=()=>openDoubleChooser();
$('#doubleClose').onclick=()=>$('#doubleDialog').close();
...
function renderHand(){
  ...
  // V17: as DUAS cartas da dupla recebem destaque e botão ×2. Na V16
  // apenas a primeira carta física recebia o botão, que podia ficar escondido
  // pela sobreposição visual da mão.
  ...
  const doubleBtn=$('#doubleActionBtn');
  doubleBtn.classList.toggle('hidden',state.status!=='playing');
  doubleBtn.disabled=!(myTurn&&!state.paused&&!state.continuationPlayerId&&doubleOpportunity);
  doubleBtn.textContent=doubleOpportunity?`🃏🃏 Carta Dupla (${state.me.doublePairs.length})`:'🃏🃏 Carta Dupla';
  doubleBtn.title=doubleOpportunity?'Você tem uma dupla válida. Clique para escolher e jogar as duas cartas juntas.':'Nenhuma dupla comum válida nesta jogada.';
...
function openDoubleChooser(){
  const pairs=state?.me?.doublePairs||[];
  if(!canAct()||state?.paused)return toast('Aguarde a sua vez para usar Carta Dupla.');
  if(!pairs.length)return toast('Você não possui uma Carta Dupla válida agora.');
  if(pairs.length===1){playDouble(pairs[0]);return;}
  const box=$('#doubleChoices');box.innerHTML='';
  pairs.forEach(pair=>{
    const c=state.me.hand.find(x=>x.id===pair.cardIds[0]);
    if(!c)return;
    const btn=document.createElement('button');
    btn.className='double-choice-btn';
    btn.innerHTML=`<span class="double-choice-cards">${c.rank}${suitGlyph[c.suit]} + ${c.rank}${suitGlyph[c.suit]}</span><span>Jogar esta dupla</span>`;
    btn.onclick=()=>{$('#doubleDialog').close();playDouble(pair)};
    box.appendChild(btn);
  });
  $('#doubleDialog').showModal();
}
